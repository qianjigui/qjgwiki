...
#数据结构#
  1. 寻找公共祖先: [http://www.cppblog.com/myjfm/archive/2011/05/12/146282.aspx 寻找最近公共祖先]
  ...
  1. 背包问题
  1. 二分搜索: [http://blog.csdn.net/cxh342968816/article/details/6672537 递减偏移数组]
  1. 栈与队列: [http://daimajishu.iteye.com/blog/1091066 用两个栈实现一个队列的功能&&两个队列实现一个栈的功能]
...
考虑采取分而治之的方法。
  ...
  1. 遍历文件a，对每个url求取Hash，然后根据所取得的值将url分别存储到1000个小文件（记为A）中。这样每个小文件的大约为300M。
